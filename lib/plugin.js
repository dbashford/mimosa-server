"use strict";
var connections, currentServer, engines, express, fs, logger, registration, startProvidedServer, _, __cleanUpConnections, _startDefaultServer, _startProvidedServer, _startServer;
...
_startDefaultServer = function(config, options, done) {
  ...
  currentServer.on('request', function(request, response) {
    return connections.push(request);
  });
      ...
      return app.get('/:viewname*/?', function(req, res) {
        return res.render(req.params.viewname, options, function(err, html) {
          if (err) {
            ...
            return res.send(404, "Cannot GET /" + req.params.viewname);
          } else {
