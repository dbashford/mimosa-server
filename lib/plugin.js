"use strict";
var connections, currentServer, engines, express, fs, logger, path, registration, startProvidedServer, transpilers, _, __cleanUpConnections, _removeFromConnections, _startDefaultServer, _startProvidedServer, _startServer;
...
_startProvidedServer = function(config, options, done) {
  ...
  if (((_ref = config.server.packageJSON) != null ? _ref.dependencies : void 0) != null) {
    ...
    _.intersection(deps, transpilers).forEach(function(transpiler) {
      var transp;
      if (logger.debug && logger.isDebug()) {
